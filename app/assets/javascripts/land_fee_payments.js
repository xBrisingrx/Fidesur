let land_fee_payment_data = {
	...
	calc_valor_en_pesos(name){
		...
		if (currency == 2 || currency == 3) {
			...
			let land_fee_payment_value = document.getElementById(`${name}_payment`).value
			document.getElementById(`${name}_value_in_pesos`).value = 1
			document.getElementById(`${name}_calculo_en_pesos`).value = ( !isNaN( land_fee_payment_value ) ) ? land_fee_payment_value : 0.0
		}
	},
	check_input( event, input, atributo ) {
		...
		this.calc_valor_en_pesos('land_fee_payment')
	}
}